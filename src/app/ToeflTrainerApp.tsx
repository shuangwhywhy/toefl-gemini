            ...
            try {
                ...
                recorder.onstop = async () => {
                    ...
                    try {
                        ...
                        setMessages(prev => [...prev, { id: Date.now(), role: 'model', text: "语音转文字失败，请稍后再试。" }]);
                    }
                };
                recorder.start();
                setIsListening(true);
            } catch (error) {
                setMessages(prev => [...prev, { id: Date.now(), role: 'model', text: "抱歉，无法访问麦克风进行语音输入。" }]);
            }
        }
    };
...
当前学生正在练习的上下文数据: "${contextText}"`
            ...
            if (!chatScope.isSessionCurrent(sendSession)) return;
                                        ...
                                        if (token.type === 'shown') {
                                            ...
                                            return (
                                                <span key={idx} className={`relative inline-block ${spacing}`}>
                                                    <input
                                                        ref={el => {
                                                            inputRefs.current[token.gapIdx] = el;
                                                        }}
                                                        type="text"
...
function ShadowingModule({ onBack }) {
    ...
    useEffect(() => {
        ...
        const fetchCurrentTTS = async () => {
            ...
            if (!audioCacheRef.current[selectedVoice]) {
                ...
                if (!isCancelled && typeof url === 'string') { audioCacheRef.current[selectedVoice] = url; setTtsAudioUrl(url); }
    ...
    const generateNewText = async (targetLengthLevel = lengthLevel, currentFocus = learningFocus, currentDifficultyLevel = difficultyLevel) => {
      ...
      CRITICAL INSTRUCTION: Output ONLY the actual English sentence as the value for the "sentence" key. DO NOT include any conversational filler like "Here is the sentence:" inside the JSON.`;
            ...
            let data = await fetchGeminiText(prompt, 0.7, 400, schema, null, validator, {
                scopeId: requestScope.scopeId,
                supersedeKey: 'shadow:generate'
            });
            if (signal.aborted || !requestScope.isSessionCurrent(session)) return;
        ...
        if (isRecording) {
            ...
            try {
                ...
                mediaRecorderRef.current.onstop = async () => {
                    ...
                    if (!requestScope.isSessionCurrent(recordingSession)) return;
                    setTranscribedText(transcript);
                    evaluatePronunciation(text, transcript, audioBlob);
                };
    ...
    const evaluatePronunciation = async (originalText, spokenText, audioBlob) => {
      ...
      Return JSON: {"errors": [{"index": ..., "word": "...", "status": "omitted|wrong", "spoken": "...", "ipa": "..."}], "advice": "...", "fluencyScore": 0-100, "intonationScore": 0-100, "suggestedFocus": "..."}`;
            ...
            const parts = [{ text: promptText }] as Array<Record<string, unknown>>;
            if (base64Audio) {
            ...
            if (!requestScope.isSessionCurrent(evaluationSession)) return;
...
function InterviewModule({ onBack }) {
    ...
    const handleInterviewerAudioEnded = async () => {
        ...
        try {
            ...
            mediaRecorderRef.current.onstop = async () => {
                ...
                if (!requestScope.isSessionCurrent(answerSession)) return;
    ...
    Return JSON: {"score": 0-30, "timeAnalysis": "...", "speedAnalysis": "...", "overallFeedback": "...", "detailedAnalysis": [{"question": "...", "feedback": "...", "tailoredResponse": [{"strategy": "...", "text": "..."}]}]}`;
        ...
        try {
            let data = await fetchGeminiText(parts, 0.4, 4000, schema, null, null, {
                scopeId: requestScope.scopeId,
                ...
                {status === 'setup' && (
                    <div className="space-y-6">
                        <div className="bg-white rounded-2xl shadow-sm p-10 text-center flex flex-col items-center">
                            ...
                            <button onClick={generateInterview} disabled={String(status) === 'generating'} className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-full font-bold shadow-lg shadow-blue-200 transition-all transform hover:scale-105 flex items-center disabled:opacity-50 disabled:scale-100">
                                {String(status) === 'generating' ? <RefreshCw className="w-5 h-5 mr-2 animate-spin" /> : null}
                                {String(status) === 'generating' ? "考卷生成中..." : "生成考卷并入座"} <ChevronRight className="w-5 h-5 ml-2" />
                            </button>
                        </div>
                        {renderHistory()}
                    </div>
